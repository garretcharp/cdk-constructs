...
function promisifiedRequest(url, options, modifyRequest = () => {}) {
	return new Promise((resolve, reject) => {
		const request = https.request(url, options, response => {
			...
			response.on('end', () => {
				...
				return statusCode >= 200 && statusCode < 300 ? resolve({ statusCode, headers, data }) : reject('Request failed with status code: ' + statusCode);
			});
		});
...
async function sendResponse (event, Status, Reason, PhysicalResourceId, Data) {
...
async function createOrUpdateIndex(event) {
...
exports.handler = async (event) => {
	...
	try {
		...
		return sendResponse(event, 'FAILED', error.message || 'Internal Error', PhysicalResourceId, {});
	}
};
